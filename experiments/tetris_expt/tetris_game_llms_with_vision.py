...
# Initialize results storage, now including average response time per trial
results = {
    # "gpt-4.1": {
    #     "final_score": [],
    #     "max_stack_height": [],
...
def run_tetris(method, save_path="demo_", save_type=".json", rng=random.Random(42), auto_gen_path=False):
    ...
    # initialize agent and history trackers
    ag = LLM_Agent(model=method)
    game_history = []
    resp_times = []
    cnt = 0

    while tetris.running and cnt < MAX_EPOCH:
        cnt += 1
        screen.fill(BLACK)
    ...
    # save the detailed game history
...
if __name__ == "__main__":
    # run experiments and collect metrics including avg response time
