...
if __name__ == "__main__":
    ...
    for model in models:
        ...
        run_model_on_questions(model, ques, result_path_plain, with_perception=False, max_questions=3)

        # WITH PERCEPTION
        ques = copy.deepcopy(physics_questions)
        result_path_apex = f"./experiments/physical_question_expt/results/{model.replace('/', '_').replace(':','_')}_physics_results_final_APEX.json"
        run_model_on_questions(model, ques, result_path_apex, with_perception=True, max_questions=3)
