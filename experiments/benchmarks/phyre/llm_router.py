...
def call_llm(model, messages, img_url=None):
    """
    General model interface with optional image input.
    - model: model identifier
    - messages: OpenAI-style chat message list
    - img_url: optional, a public image URL or a base64 data-url
    """
    # If img_url is provided, inject it into the last user message
    if img_url:
        if not messages or messages[-1].get("role") != "user":
            raise ValueError("The last message must be from role 'user' to attach an image.")
        # Extract plain text
    ...
    # Claude multimodal, using native image/message schema
    elif model.startswith('claude-'):
        client = Anthropic()
        real_msgs = []
        for m in messages:
            if m['role'] == 'system':
                # Claude SDK doesn’t support a separate system field, inject as first user message
