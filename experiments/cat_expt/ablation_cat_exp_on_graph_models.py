...
def run_exp(difficulty="Simple", method='APEX', model='gpt-4o-mini', run_callback=None, top_k=5,
    ...
    apex = None
    if method == 'APEX':
        danger_model, model_addr = None, ''
        if graph_model == 'graphormer':
            ...
            model_addr = 'model/diffgraphormer_physics.pt'

        if graph_model == 'gat':
            danger_model = DiffGAT(
                in_feats=7,
                hidden_dim=32,
                dropout=0.3
            )
            model_addr = 'model/diffgat_physics.pt'

        if graph_model == 'gcn':
            danger_model = DiffGCN(
                in_feats=7,
                hidden_dim=32,
                dropout=0.3
            )
            model_addr = 'model/diffgcn_physics.pt'

        danger_model.load_state_dict(torch.load(model_addr, map_location='cpu'))
        danger_model.eval()  # Turn off dropout
        apex = APEX(graphormer_model=danger_model, physics_simulator="mujoco", llm_agent=agent, dt=dt,
                    available_move=available_move, top_k=top_k)
...
def run_trial(difficulty, method, model, trial_id, top_k, graph_model):
    ...
    collision_flag = run_exp(difficulty, method=method, model=model, run_callback=callback_fn, top_k=top_k,
...
if __name__ == "__main__":
    ...
    NUM_TRIALS = 5
    difficulties = ["Hard"]
    top_graph_models = ['gat', 'gcn', 'graphormer']
    methods = {"APEX": ['gpt-4o', 'gpt-4o-mini']}
    results = {
        gm: {
            d: {
                m: {
        ...
        } for gm in top_graph_models
    }
    save_path = "results/results_diff_graph_models.json"
            ...
            for method in methods:
                for model in methods[method]:
                    for i in range(NUM_TRIALS):
                        result = run_trial(difficulty, method, model, i, 5, gm)
                        results[gm][difficulty][method][model]["ast"].append(result["survival_time"])
                        results[gm][difficulty][method][model]["cfr"] += int(not result["collision_flag"])

                        if result["actions_len"] > 0:
                            results[gm][difficulty][method][model]["iar"] += result["invalid_actions"] / result[
                                "actions_len"] / NUM_TRIALS
                            results[gm][difficulty][method][model]["latency"].append(
                                result["latency_sum"] / result["actions_len"])
                        else:
                            results[gm][difficulty][method][model]["iar"] += 0
                            results[gm][difficulty][method][model]["latency"].append(0.0)
    ...
    for difficulty in difficulties:
        for gm in top_graph_models:
            for method in methods:
                for model in methods[method]:
                    summary = results[gm][difficulty][method][model]
                    ...
                    print(f"[{difficulty}][{method}][{model}][{gm}] CFR={cfr_rate:.2f} | AST={avg_ast:.2f}s | "
